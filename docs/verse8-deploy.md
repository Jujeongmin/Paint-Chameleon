...
**에디터 상단 바의 Launch 버튼을 누르십시오.** 플랫폼이 배포를 직접
처리합니다. 출시된 게임은 My Creations에서 관리하고, 고친 뒤에는 Edit Game →
다시 Launch로 갱신합니다.

Launch는 워크스페이스의 현재 상태를 배포하므로, **먼저 푸시하고 나서**
누르십시오.

### CLI 배포는 사용자 경로가 아닙니다
...
에이전트가 여기서 무한 재시도에 빠진 적이 있습니다 — 401은 재시도로 풀리는
종류가 아니라는 것을 에이전트가 모릅니다.

`--preview` / `--prod` 는 `V8_ACCESS_TOKEN` 을 Bearer로 보내지만, **그 토큰을
크리에이터가 발급받는 경로가 문서에 없습니다.** 워크스페이스 환경에도 들어
있지 않습니다. 플랫폼의 출시 파이프라인이 서버 측에서 주입하는 값으로
보입니다. 그러니 CLI로 배포하려 하지 마십시오.
...
배포는 하지 마. CLI 배포는 401이고 사용자 경로가 아니야 — 내가 에디터의
Launch 버튼으로 직접 할게.
```
